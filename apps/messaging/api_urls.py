...
urlpatterns = [
    ...
    path(
        "customers/suggested_merges/", api_views.suggested_merges, name="customer-suggested-merges"
    ),
    ...
    path(
        ...
        api_views.customer_timeline,
        name="customer-timeline",
    ),
    path(
        "customers/<uuid:customer_id>/identities/<uuid:identity_id>/refresh/",
        api_views.refresh_customer_identity,
        name="customer-identity-refresh",
    ),
    # ---- Connected channels -------------------------------------------
    path("channels/", api_views.ConnectedAccountListView.as_view(), name="channels"),
    path(
        "channels/<uuid:id>/", api_views.ConnectedAccountDetailView.as_view(), name="channel-detail"
    ),
    path(
        "channels/<uuid:channel_id>/settings/", api_views.account_settings, name="channel-settings"
    ),
    path(
        "channels/<uuid:channel_id>/credentials/",
        api_views.update_account_credentials,
        name="channel-update-credentials",
    ),
    ...
    path(
        "admin/channels/<uuid:channel_id>/toggle/",
